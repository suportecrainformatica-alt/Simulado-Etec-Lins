...
export default function App() {
  ...
  // - "estudo": Mostra se está certo/errado na hora que clica, com explicação inline imediata.
        ...
        <AnimatePresence mode="wait">
                ...
                {questoesFiltradas.length === 0 ? (
                  ...
                  <div className="space-y-5">
                    {questoesFiltradas.map((questao, qIndex) => {
                      ...
                      const hasGraded = modoEstudo ? !!userChoice : provaEntregue;
                        ...
                        >
                          ...
                          <div className="space-y-2.5">
                            {Object.entries(questao.alternativas).map(([letra, textoOption]) => {
                              ...
                              let btnColor = "border-slate-20s bg-slate-50 hover:bg-slate-100/90 hover:border-slate-320 text-slate-700";
                              ...
                              if (hasGraded) {
                                if (correctAnsThis) {
                                  ...
                                  btnColor = "border-rose-450 bg-rose-50 text-rose-900 cursor-not-allowed font-medium ring-1 ring-rose-500/20";
                                ...
                                btnColor = "border-indigo-600 bg-indigo-50 text-indigo-900 ring-2 ring-indigo-500/10 font-semibold";
                              }
                          ...
                          {/* EXPLICAÇÃO DO CADERNO (APARECE INLINE SÓ QUANDO REVELADA) */}
                          {hasGraded && questao.explicacao && (
                            <motion.div
                              initial={{ opacity: 0, scale: 0.98 }}
                              animate={{ opacity: 1, scale: 1 }}
                              className="mt-5 p-4 bg-slate-50 border border-slate-200 rounded-xl text-xs md:text-sm text-slate-650"
                            >
                              <div className="font-extrabold text-slate-900 flex items-center gap-1.5 mb-1.5 font-display text-xs uppercase tracking-wider">
                                <Info className="h-4 w-4 text-indigo-600" />
                                Gabarito Comentado
                              </div>
                              <p className="leading-relaxed whitespace-pre-wrap">{questao.explicacao}</p>
                            </motion.div>
                          )}
          ...
          {currentTab === "professor" && (
            ...
            >
              ...
              {/* CORE EDITOR JSON DE QUEStOES (COLS: 8) */}
              <div className="lg:col-span-8 bg-white rounded-2xl p-5 md:p-6 border border-slate-200 shadow-sm flex flex-col gap-4">
                <div>
